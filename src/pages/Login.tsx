...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const redirectTo =
        (location.state && (location.state as any).from?.pathname) || "/dashboard";
      navigate(redirectTo, { replace: true });
    } catch (err: any) {
      // err may be APIError from client or generic Error
      ...
      <div className="w-full max-w-md">
        <Card className="bg-background/95 backdrop-blur border-border/50 shadow-elegant">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold text-gradient-primary">Welcome back to Nota</CardTitle>
